...
requireMatch(html.includes('const LSC_BUILD = \'179\';'), 'Build 179 config is not present');
...
requireMatch(html.includes("String(LSC_BUILD) === '179'") && html.includes('if(QA_TEST_ACCESS)return meta.energyMax;') && html.includes('if(QA_TEST_ACCESS)return true;'), 'Build 179 temporary QA energy override is missing');
...
requireMatch(html.includes('BUILD 179 · QA TEST ACCESS') && html.includes('REPEATABLE OPERATIONS'), 'Build 179 QA disclosure is missing');
...
requireMatch(html.includes('lsc179-research-style') && html.includes('l179-doctrine-grid') && html.includes('data-research-select') && html.includes('l179-detail-panel'), 'Build 179 compact three-doctrine Research presentation is missing');
requireMatch(html.includes('l179-research-mode') && html.includes('#lsc137-app.l179-research-mode .l137-hero'), 'Build 179 research-only compact destination is missing');
requireMatch(html.includes('function researchPrimaryEffect(node)') && html.includes('function researchNodeState(node)') && html.includes('function researchStatMarkup(node)'), 'Build 179 concise node summaries are missing');
requireMatch(html.includes("name:'Attack Power'") && html.includes("name:'Range'") && html.includes("name:'Barrier Health'") && html.includes("name:'Cooldown'"), 'Build 179 short Research names are missing');
