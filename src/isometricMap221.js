...
(function(root){
 ...
 if(typeof Image!=='undefined')Object.keys(roadLayout).forEach(function(name){var im=new Image();im.onload=function(){roadMasks[name]=prepareRoad(name,im);revision++;};im.src='assets/roads/'+roadLayout[name].file;roadArt[name]=im;});
 ...
 function prepareRoad(name,im){if(typeof document==='undefined')return im;var layout=roadLayout[name],r=layout.crop,canvas=document.createElement('canvas'),ctx,data,i,red,green,blue,keep;canvas.width=512;canvas.height=256;ctx=canvas.getContext('2d',{willReadFrequently:true});ctx.drawImage(im,r[0],r[1],r[2],r[3],0,0,canvas.width,canvas.height);try{data=ctx.getImageData(0,0,canvas.width,canvas.height);for(i=0;i<data.data.length;i+=4){red=data.data[i];green=data.data[i+1];blue=data.data[i+2];keep=Math.max(0,Math.min(1,(red-green*.95+18)/42));if(red>170&&blue>95&&red>green*1.35)keep=0;if(name==='bridge'&&blue>red*1.05&&blue>green*.92)keep=0;data.data[i+3]=Math.round(data.data[i+3]*keep);}ctx.putImageData(data,0,0);}catch(e){return im;}return canvas;}
 ...
 function paint(g,m,v,c,selected,path,now,cache){
  ...
  function drawPreparedRoad(im,layout,p,rotation){var a=[[1,0,0,1],[0,.5,-2,0],[-1,0,0,-1],[0,-.5,2,0]][rotation||0],prepared=roadMasks[layout===roadLayout.straight?'straight':layout===roadLayout.corner?'corner':layout===roadLayout.tee?'tee':layout===roadLayout.cross?'cross':layout===roadLayout.bridge?'bridge':'end'];g.translate(p.x,p.y);g.transform(a[0],a[1],a[2],a[3],0,0);if(prepared&&prepared!==im)g.drawImage(prepared,-s/2,-s/4,s,s/2);else{var r=layout.crop;g.drawImage(im,r[0],r[1],r[2],r[3],-s/2,-s/4,s,s/2);}}
  function roadEnd(p,mask){var im=roadArt.straight,layout=roadLayout.straight;if(!im||!im.complete||!im.naturalWidth)return false;var points={1:[[0,0],[s/2,0],[0,s/4]],2:[[0,0],[-s/2,0],[0,-s/4]],4:[[0,0],[0,s/4],[-s/2,0]],8:[[0,0],[s/2,0],[0,-s/4]]}[mask]||[[0,0],[s/2,0],[0,s/4]],rotation=(mask===4||mask===8)?1:0;g.save();diamond(g,p,s+.8);g.clip();g.beginPath();points.forEach(function(q,i){g[i?'lineTo':'moveTo'](p.x+q[0],p.y+q[1]);});g.closePath();g.clip();drawPreparedRoad(im,layout,p,rotation);g.restore();g.save();g.fillStyle='#d2a667';g.strokeStyle='#9d7041';g.lineWidth=Math.max(1,s*.018);g.beginPath();g.arc(p.x,p.y,s*.065,0,Math.PI*2);g.fill();g.stroke();g.restore();return true;}
  function roadPiece(p,mask,water){var plan=roadPlan(mask,water);if(!plan)return false;if(plan[0]==='end')return roadEnd(p,mask);var im=roadArt[plan[0]],layout=roadLayout[plan[0]];if(!im||!im.complete||!im.naturalWidth||!layout)return false;g.save();diamond(g,p,s+.8);g.clip();drawPreparedRoad(im,layout,p,plan[1]||0);g.restore();return true;}
