// Build 142 — combat-source readability and first battlefield visual pass.
(function () {
  'use strict';
  if (window.__LSC_COMMAND_BASE_142__) return;
  window.__LSC_COMMAND_BASE_142__ = true;
  ...
  function createRun(phaseOverride) {
    ...
    return { active:true, paused:false, complete:false, phase:phase, elapsed:0, speed:1, assault:1, assaultElapsed:0, assaultSpawned:0, assaultKills:0, assaultTargets:targets, transition:0, spawn:0, spawned:0, kills:0, xp:0, xpNext:30, level:1, bossSpawned:false, bossDefeated:false, upgradeOpen:false, abilityCd:0, lastHit:0,
      ...
      enemies:[],bullets:[],particles:[],damage:{commander:0,turret:0,squad:0,artillery:0},feedback:null };
  }
  function enemy(kind) {
  ...
  function fire(o,t,damage){if(!t)return;var x=t.x-o.x,y=t.y-o.y,l=Math.hypot(x,y)||1,source=o.source||'squad',speed=source==='turret'?390:source==='commander'?650:500;run.bullets.push({x:o.x,y:o.y,px:o.x,py:o.y,vx:x/l*speed*dpr(),vy:y/l*speed*dpr(),damage:damage,life:.9,source:source,color:source==='commander'?'#fff07a':source==='turret'?'#ff8a2a':'#8edcff'});}
  ...
  function drawTurret(t){var s=dpr(),target=nearest(t,t.range),ang=target?Math.atan2(target.y-t.y,target.x-t.x):0;ctx.save();ctx.translate(t.x,t.y);ctx.fillStyle='#4b3320';ctx.strokeStyle='#ff8a2a';ctx.lineWidth=2*s;ctx.fillRect(-11*s,-6*s,22*s,15*s);ctx.strokeRect(-11*s,-6*s,22*s,15*s);ctx.rotate(ang);circle(0,-6*s,8*s,'#6c4a23','#ffb14a',2*s);ctx.fillStyle='#ff8a2a';ctx.fillRect(3*s,-9*s,24*s,6*s);ctx.restore();ctx.fillStyle='#ffb14a';ctx.font='bold '+7*s+'px "Share Tech Mono"';ctx.textAlign='center';ctx.fillText('TURRET',t.x,t.y+20*s);}
