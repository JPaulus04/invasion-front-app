...
(function () {
  ...
  var CAMPAIGN_REPLAY_CREDIT_RATE = .40;
  ...
  var HQ_LEVEL_CAP = 10;
  var HQ_LEVELS = [
    {level:1,name:'FIELD COMMAND POST',phase:0,cost:{credits:0,parts:0},feature:'INITIAL COMMAND NODE'},
    {level:2,name:'REINFORCED COMPOUND',phase:0,cost:{credits:250,parts:0},feature:'REINFORCED PERIMETER'},
    {level:3,name:'FORTIFIED HEADQUARTERS',phase:0,cost:{credits:500,parts:0},feature:'RAISED CORNER BASTIONS'},
    {level:4,name:'ARMORED CITADEL',phase:0,cost:{credits:750,parts:0},feature:'ARMORED WALL COURSE'},
    {level:5,name:'COMMAND FORTRESS',phase:0,cost:{credits:1000,parts:0},feature:'FORTRESS ENERGY GRID'},
    {level:6,name:'HEAVY BULWARK',phase:14,cost:{credits:2000,parts:8},feature:'+100 HQ · +15 BARRIER CAPACITY'},
    {level:7,name:'AUXILIARY BATTERY',phase:18,cost:{credits:3000,parts:12},feature:'WEST DEFENSE HARDPOINT'},
    {level:8,name:'FIELD REPAIR COMPLEX',phase:23,cost:{credits:4500,parts:18},feature:'STRUCTURE REPAIRS BETWEEN ASSAULTS'},
    {level:9,name:'TWIN BATTERY FORTRESS',phase:29,cost:{credits:6500,parts:26},feature:'EAST DEFENSE HARDPOINT'},
    {level:10,name:'LAST STAND COMMAND',phase:36,cost:{credits:9000,parts:36},feature:'EMERGENCY LAST STAND PROTOCOL'}
  ];
  var HQ_TIER_NAMES = HQ_LEVELS.map(function(level){return level.name;});
  function hqDefinition(level){return HQ_LEVELS[Math.max(0,Math.min(HQ_LEVELS.length-1,Math.floor(Number(level)||1)-1))];}
  function hqProfile(level){level=Math.max(1,Math.min(HQ_LEVEL_CAP,Math.floor(Number(level)||1)));return{bonusHq:level>=6?100:0,bonusBarrier:level>=6?15:0,auxiliary:level>=9?2:level>=7?1:0,repairHq:level>=8?25:0,repairBarrier:level>=8?14:0,lastStand:level>=10,power:level>=10?190:level>=9?145:level>=8?100:level>=7?65:level>=6?30:0};}
  ...
  var EQUIPMENT_CATALOG = [
    ...
    {id:'weapon-siege-rifle',slot:'weapon',rarity:'epic',minPhase:8,name:'Siege Rifle',effectText:'Commander damage +18%; boss damage +22%; armor damage +35%.',power:48,effects:{commanderDamage:.18,commanderBossDamage:.22,commanderArmorDamage:.35}},
  ...
  var EQUIPMENT_COMPARE_EFFECTS = [
    ...
    {key:'commanderArmorDamage',label:'COMMANDER ARMOR DAMAGE',kind:'percent'},
  ...
  function defaults() { return { settlementMode:204, releaseSchema:RELEASE_SCHEMA, credits: BALANCE.ECONOMY.startingCredits, parts: BALANCE.ECONOMY.startingParts, phase: 1, bestPhase: 0, commander: 1, commanderSchema:COMMANDER_SCHEMA, commanderNotice:null, research: 0, researchSchema:RESEARCH_SCHEMA, researchNodes:{}, researchPoints:0, legacyResearchLevels:0, legacyResearchDamage:0, hq: 1, phaseLosses: {}, equipmentSchema:EQUIPMENT_SCHEMA, equipmentBalanceSchema:186, equipment:[], equipped:{weapon:null,rig:null,module:null}, equipmentNextId:1, equipmentLegendaryMisses:0, equipmentNotice:null, energySchema:ENERGY_SCHEMA, energy:ENERGY_MAX, energyMax:ENERGY_MAX, energyUpdatedAt:Date.now(), campaignRetryPhase:null, operationSchema:OPERATION_SCHEMA, operationLevel:1, operationManualBest:0, junkyardLevel:1, junkyardManualBest:0, operationLastClearDay:'' }; }
  ...
  function loadMeta() {
    try {
      ...
      loaded.settlementMode=source.phase?(source.settlementMode===204?204:203):204;
      ...
      loaded.hq=guardedInteger(loaded.hq,1,1,HQ_LEVEL_CAP);
      ...
      loaded.equipmentLegendaryMisses=guardedInteger(loaded.equipmentLegendaryMisses,0,0,7);
      ...
      if(Math.max(0,Number(source.equipmentBalanceSchema)||0)<186&&loaded.bestPhase>=5){
        var earnedCache=[];
        function ownsRarity(rarity){return loaded.equipment.some(function(instance){var item=equipmentDefinition(instance.itemId);return item&&item.rarity===rarity;});}
        function grantVeteranEquipment(itemId){var definition=equipmentDefinition(itemId);if(!definition||loaded.equipment.some(function(instance){return instance.itemId===itemId;}))return;var uid='eq-'+loaded.equipmentNextId++;loaded.equipment.push({uid:uid,itemId:itemId,acquiredPhase:loaded.bestPhase,acquiredAt:Date.now(),locked:true,source:'BUILD 186 VETERAN CACHE'});earnedCache.push(definition.name);}
        if(!ownsRarity('epic'))grantVeteranEquipment(loaded.bestPhase>=8?'weapon-siege-rifle':'weapon-stabilized-carbine');
        if(loaded.bestPhase>=15&&!ownsRarity('legendary'))grantVeteranEquipment('weapon-last-word');
        if(earnedCache.length)loaded.equipmentNotice={type:'balance-cache',items:earnedCache};
      }
      loaded.equipmentBalanceSchema=186;
      ...
      // Retired in Build 188: every campaign deployment consumes energy.
      loaded.campaignRetryPhase=null;
  ...
  function renderCampaignControls(panel){
    ...
    section.innerHTML='<h3>CAMPAIGNS</h3><p class="l137-copy">Progress saves automatically to your current campaign. New Campaign starts a separate world. Saves are on this device.</p><input data-name maxlength="40" placeholder="Campaign name" aria-label="Campaign name" style="width:100%;box-sizing:border-box;padding:12px;margin:8px 0"><button class="l137-btn" data-new>NEW SETTLEMENT CAMPAIGN</button><div data-confirm></div><div data-saves style="max-height:330px;overflow:auto"></div><p data-notice role="status" class="l137-copy"></p>';panel.appendChild(section);
    ...
    function finish(result){if(!result.ok){notice.textContent=result.reason;return;}meta=loadMeta();selectedCampaignPhase=meta.phase;selectedResearchNodeId=null;refreshHeader();renderTab('reclamation');}
  ...
  function equipmentEffects(){
    var result={commanderDamage:0,commanderRate:0,commanderBossDamage:0,commanderArmorDamage:0,turretDamage:0,turretRate:0,turretRange:0,hqHp:0,barrierHp:0,assaultHqRepair:0,assaultBarrierRepair:0,hqDamageReduction:0,barrierDamageReduction:0,artilleryDamage:0,artilleryCooldown:0,fieldXp:0};
    EQUIPMENT_SLOTS.forEach(function(slot){
  ...
  function equipmentDropRarity(phase,firstClear){
    var ownsLegendary=meta.equipment.some(function(instance){var item=equipmentDefinition(instance.itemId);return item&&item.rarity==='legendary';});
    if(firstClear&&phase===5)return'epic';
    if(firstClear&&phase===10)return Math.random()<.12?'legendary':'epic';
    if(firstClear&&phase===15&&!ownsLegendary)return'legendary';
    if(firstClear&&phase>15&&meta.equipmentLegendaryMisses>=7)return'legendary';
    var roll=Math.random(),legendaryChance=phase>=20?.12:phase>=15?.09:phase>=10?.04:phase>=8?.02:0,epicChance=phase>=20?.52:phase>=15?.47:phase>=10?.41:phase>=8?.30:phase>=4?.18:0;
    if(roll<legendaryChance)return'legendary';
    if(roll<legendaryChance+epicChance)return'epic';
    return firstClear&&phase>=5&&phase%5===0?'epic':firstClear&&phase>=10?'epic':'common';
  }
  ...
  function awardEquipmentDrop(phase,firstClear){
    ...
    if(firstClear&&phase>=15)meta.equipmentLegendaryMisses=rarity==='legendary'?0:Math.min(7,meta.equipmentLegendaryMisses+1);
    award.firstClear=firstClear;
    award.milestone=firstClear&&phase%5===0;
    return award;
  }
  ...
  function installStyles() {
    ...
    s.textContent =
      ...
      '.l137-hero{height:34%;min-height:180px;display:flex;align-items:center;justify-content:center;position:relative}.l137-hq-art{width:150px;height:102px;position:relative;border:2px solid #9a8653;border-radius:18px;background:linear-gradient(145deg,rgba(41,75,87,.75),rgba(7,17,23,.96));box-shadow:0 0 42px rgba(34,212,255,.18),inset 0 -20px 32px #071117;transition:width .3s,height .3s,border .3s,box-shadow .3s}.l137-hq-art[data-tier="2"]{width:162px;height:112px;border-width:3px}.l137-hq-art[data-tier="3"]{width:176px;height:124px;border-width:4px;border-color:#d4b45e}.l137-hq-art[data-tier="4"]{width:188px;height:134px;border-width:5px;border-color:#e0c56d}.l137-hq-art[data-tier="5"]{width:202px;height:144px;border-width:6px;border-color:#7ef8ff}.l137-hq-art[data-tier="6"],.l137-hq-art[data-tier="7"],.l137-hq-art[data-tier="8"],.l137-hq-art[data-tier="9"],.l137-hq-art[data-tier="10"]{width:218px;height:154px;border-width:8px;border-color:#91f4ff;box-shadow:0 0 78px rgba(65,229,255,.42),inset 0 0 0 3px #314d51}.l137-hq-art[data-tier="8"],.l137-hq-art[data-tier="9"],.l137-hq-art[data-tier="10"]{border-color:#ffd166}.l137-hq-art[data-tier="10"]{box-shadow:0 0 94px rgba(255,209,102,.48),inset 0 0 0 4px #3f6065}.l165-hq-wall{position:absolute;inset:13px 10px 8px;border:2px solid #806d4a;border-radius:12px}.l137-hq-art[data-tier="2"] .l165-hq-wall{border-width:4px}.l137-hq-art[data-tier="3"] .l165-hq-wall,.l137-hq-art[data-tier="4"] .l165-hq-wall{border-width:7px}.l137-hq-art[data-tier="5"] .l165-hq-wall,.l137-hq-art[data-tier="6"] .l165-hq-wall,.l137-hq-art[data-tier="7"] .l165-hq-wall,.l137-hq-art[data-tier="8"] .l165-hq-wall,.l137-hq-art[data-tier="9"] .l165-hq-wall,.l137-hq-art[data-tier="10"] .l165-hq-wall{border:10px double #70cbd2;box-shadow:inset 0 0 0 3px #274a50}.l165-hq-core{position:absolute;left:50%;top:51%;width:68px;height:52px;transform:translate(-50%,-50%);display:grid;place-items:center;border:2px solid #ffd166;border-radius:14px;background:linear-gradient(145deg,#426467,#172a30);box-shadow:0 8px 15px rgba(0,0,0,.34)}.l165-hq-core span{font-size:23px;font-weight:900}.l137-hq-art[data-tier="5"] .l165-hq-core,.l137-hq-art[data-tier="6"] .l165-hq-core,.l137-hq-art[data-tier="7"] .l165-hq-core,.l137-hq-art[data-tier="8"] .l165-hq-core,.l137-hq-art[data-tier="9"] .l165-hq-core,.l137-hq-art[data-tier="10"] .l165-hq-core{width:100px;height:76px;border-color:#9cecff}.l165-hq-mast{position:absolute;left:50%;bottom:76%;width:5px;height:34px;transform:translateX(-50%);border-radius:5px;background:#9cecff;box-shadow:0 0 10px #22d4ff}.l165-hq-tower{display:none;position:absolute;width:18px;height:18px;border:2px solid #d7bd75;border-radius:4px;background:#2c4143}.l137-hq-art[data-tier]:not([data-tier="1"]):not([data-tier="2"]) .l165-hq-tower{display:block}.l165-hq-tower.nw{left:-9px;top:-9px}.l165-hq-tower.ne{right:-9px;top:-9px}.l165-hq-tower.sw{left:-9px;bottom:-9px}.l165-hq-tower.se{right:-9px;bottom:-9px}.l188-hq-hardpoint,.l188-hq-repair{display:none;position:absolute;z-index:4}.l188-hq-hardpoint{top:47%;width:20px;height:20px;border:3px solid #8fefff;border-radius:50%;background:#173943;box-shadow:0 0 12px #22d4ff}.l188-hq-hardpoint.west{left:-20px}.l188-hq-hardpoint.east{right:-20px}.l137-hq-art[data-tier="7"] .l188-hq-hardpoint.west,.l137-hq-art[data-tier="8"] .l188-hq-hardpoint.west,.l137-hq-art[data-tier="9"] .l188-hq-hardpoint,.l137-hq-art[data-tier="10"] .l188-hq-hardpoint{display:block}.l188-hq-repair{left:50%;bottom:-13px;transform:translateX(-50%);width:54px;height:15px;border:2px solid #ffd166;border-radius:4px;background:#253e40}.l137-hq-art[data-tier="8"] .l188-hq-repair,.l137-hq-art[data-tier="9"] .l188-hq-repair,.l137-hq-art[data-tier="10"] .l188-hq-repair{display:block}.l137-hq-lv{position:absolute;bottom:7px;font:9px "Share Tech Mono",monospace;color:#9cecff;letter-spacing:.4px}' +
      ...
      '.l186-equipped-badge{display:inline-block!important;margin:5px auto 0!important;width:max-content;padding:2px 5px;border-radius:4px;background:#11643a;color:#7fffae!important}.l167-equipped-slot.filled{color:#fff}.l167-equipped-slot.filled.common{border-color:#87969d;background:linear-gradient(145deg,#26343a,#101a20);box-shadow:inset 0 0 18px rgba(174,190,198,.09)}.l167-equipped-slot.filled.epic{border-color:#b26cff;background:linear-gradient(145deg,#452361,#1c102b);box-shadow:inset 0 0 22px rgba(178,108,255,.22),0 0 12px rgba(133,68,211,.13)}.l167-equipped-slot.filled.legendary{border-color:#ffd166;background:linear-gradient(145deg,#61470e,#241b08);box-shadow:inset 0 0 24px rgba(255,209,102,.23),0 0 13px rgba(255,190,55,.15)}.l167-equipped-slot.filled.common span{color:#cbd5d9}.l167-equipped-slot.filled.epic span{color:#d7abff}.l167-equipped-slot.filled.legendary span{color:#ffe29a}.l167-item-card.equipped{box-shadow:none}.l167-item-state{padding:3px 5px;border-radius:4px;background:rgba(17,100,58,.62)}.l167-result-drop{position:relative;overflow:hidden;padding-top:25px}.l167-result-drop:before{position:absolute;left:0;right:0;top:0;padding:4px 8px;content:"COMMON DROP";background:#65747b;color:#fff;font:700 6px "Share Tech Mono",monospace;letter-spacing:1.4px}.l167-result-drop.epic:before{content:"EPIC DROP";background:#7d3fbd}.l167-result-drop.legendary:before{content:"LEGENDARY DROP";background:#b78316;color:#fff4cf}.l186-armor-hud .l139-progress-track{border-color:#8fefff;background:#0b202b;box-shadow:0 0 16px rgba(143,239,255,.32)}.l186-armor-hud .l139-progress-fill{background:linear-gradient(90deg,#7e919a,#bcecff)!important;box-shadow:0 0 12px rgba(143,239,255,.72)!important}.l186-armor-hud .l139-progress-text{color:#bdefff}.l186-barrier-critical{animation:l186BarrierPulse .45s ease-in-out 2}@keyframes l186BarrierPulse{50%{filter:brightness(1.65) saturate(1.4)}}' +
      ...
      '#hq-upgrade-overlay{position:fixed;z-index:31000;inset:0;display:none;align-items:center;justify-content:center;padding:22px;background:rgba(0,4,8,.58);backdrop-filter:blur(2px);-webkit-backdrop-filter:blur(2px)}#hq-upgrade-overlay.show{display:flex}.hq-upgrade-modal{width:min(420px,100%);padding:18px;border:1px solid #846e36;border-radius:18px;background:rgba(9,19,26,.95);box-shadow:0 18px 54px rgba(0,0,0,.52)}.hq-upgrade-title{text-align:center;font-size:23px;font-weight:900}.hq-upgrade-sub{text-align:center;color:#ffd166;font:8px "Share Tech Mono",monospace;margin-bottom:12px}.hq-upgrade-grid{display:grid;gap:8px}.hq-upgrade-choice{text-align:left;padding:12px;border:1px solid #64727a;border-radius:11px;background:rgba(18,29,35,.96);color:#fff;box-shadow:inset 3px 0 0 #7e8a90}.hq-upgrade-choice b,.hq-upgrade-choice span,.hq-upgrade-choice small{display:block}.hq-upgrade-choice b{font-size:15px}.hq-upgrade-choice span{font:8px/1.45 "Share Tech Mono",monospace;color:#b8c2c7}.hq-upgrade-choice small{margin-bottom:4px;font:7px "Share Tech Mono",monospace;letter-spacing:1.5px;color:#aab4b9}.hq-upgrade-choice.epic{border-color:#8f58d8;background:rgba(33,21,47,.96);box-shadow:inset 3px 0 0 #b477ff,0 0 14px rgba(164,92,255,.12)}.hq-upgrade-choice.epic small{color:#c99aff}.hq-upgrade-choice.legendary{border-color:#c79c32;background:rgba(48,37,14,.96);box-shadow:inset 3px 0 0 #ffd166,0 0 16px rgba(255,209,102,.14)}.hq-upgrade-choice.legendary small{color:#ffd166}' +
      ...
      '.l168-boss-hud.l186-armor-hud .l139-progress-track{border-color:#8fefff;background:#0b202b;box-shadow:0 0 16px rgba(143,239,255,.32)}.l168-boss-hud.l186-armor-hud .l139-progress-fill{background:linear-gradient(90deg,#7e919a,#bcecff)!important;box-shadow:0 0 12px rgba(143,239,255,.72)!important}.l168-boss-hud.l186-armor-hud .l139-progress-text{color:#bdefff}' +
      ...
      '.l168-boss-hud{height:61px}.l189-hq-status{display:none;margin-top:5px}.l168-boss-hud .l189-hq-status{display:block}.l189-objective-only .l189-hq-status{display:none!important}.l189-hq-text{display:flex;justify-content:space-between;color:#9fffc0;font:700 7px "Share Tech Mono",monospace}.l189-hq-track{height:7px;margin-top:3px;border:1px solid rgba(103,255,160,.65);border-radius:7px;background:#10241c;overflow:hidden}.l189-hq-fill{height:100%;background:linear-gradient(90deg,#13d867,#8dffad);box-shadow:0 0 10px rgba(60,255,130,.65);transition:width .12s linear}.l189-hq-status.warning .l189-hq-text{color:#ffd166}.l189-hq-status.warning .l189-hq-track{border-color:#ffd166}.l189-hq-status.warning .l189-hq-fill{background:linear-gradient(90deg,#d68618,#ffd166)}.l189-hq-status.critical .l189-hq-text{color:#ff725f}.l189-hq-status.critical .l189-hq-track{border-color:#ff5b48}.l189-hq-status.critical .l189-hq-fill{background:linear-gradient(90deg,#b81818,#ff5b48)}.l189-hq-status.direct{animation:l189DirectAssault .5s ease-in-out infinite alternate}@keyframes l189DirectAssault{to{filter:brightness(1.45)}}.l187-map-node.selectable{-webkit-appearance:none;appearance:none;padding:0;border:0;background:transparent;color:inherit;font:inherit;cursor:pointer}.l187-map-node.selected i{border-color:#ffd166!important;box-shadow:0 0 13px rgba(255,209,102,.42);color:#ffd166}.l189-sector-nav{display:grid;grid-template-columns:1fr 1fr;gap:6px;margin-top:8px}.l189-sector-nav button{padding:6px;border:1px solid rgba(34,212,255,.3);border-radius:7px;background:#0a1b23;color:#9cecff;font:700 7px "Share Tech Mono",monospace}.l189-sector-nav button:disabled{opacity:.28}.l189-replay-notice{margin-top:8px;padding:8px;border:1px solid rgba(255,209,102,.5);border-radius:8px;background:rgba(81,57,10,.24);color:#ffd166;font:700 7px/1.45 "Share Tech Mono",monospace;text-align:center}' +
  ...
  function installCampaignStyles(){
    if(id('lsc187-campaign-style'))return;
    var style=document.createElement('style');
    style.id='lsc187-campaign-style';
    style.textContent=
      '#lsc137-app.l187-campaign-mode .l137-hero{height:23%;min-height:126px}'+
      '#lsc137-app.l187-campaign-mode .l137-panel{padding:10px 12px 124px}'+
      '.l187-campaign-dock{display:none}.l187-campaign-mode .l187-campaign-dock{position:absolute;z-index:8;right:12px;bottom:calc(58px + env(safe-area-inset-bottom,0px));left:12px;display:flex;align-items:center;gap:8px;padding:8px;border:1px solid rgba(49,220,255,.68);border-radius:13px;background:linear-gradient(145deg,rgba(7,35,46,.98),rgba(3,14,19,.99));box-shadow:0 -10px 30px rgba(0,0,0,.55)}'+
      '.l187-dock-copy{min-width:0;flex:1;padding-left:4px}.l187-dock-copy small,.l187-dock-copy b{display:block;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.l187-dock-copy small{color:#85deee;font:5.5px "Share Tech Mono",monospace}.l187-dock-copy b{margin-top:2px;color:#fff;font-size:11px}.l187-campaign-dock .l137-deploy{width:auto;min-width:58%;margin:0;padding:11px 8px;font-size:10px}'+
      '.l187-sector-map{padding:10px;border:1px solid rgba(116,233,255,.36);border-radius:12px;background:linear-gradient(145deg,rgba(8,40,51,.9),rgba(4,15,20,.98))}.l187-sector-head{display:flex;align-items:flex-end;justify-content:space-between;gap:8px}.l187-sector-head small{color:#75d9eb;font:5.5px "Share Tech Mono",monospace}.l187-sector-head h3{margin:2px 0 0;color:#f1fbff;font-size:18px;line-height:1}.l187-sector-head strong{color:#ffd166;font:800 13px Rajdhani,sans-serif}.l187-sector-map>p{margin:6px 0;color:#96abb1;font:6.5px/1.4 "Share Tech Mono",monospace}'+
      '.l187-map-route{position:relative;display:grid;grid-template-columns:repeat(5,1fr);gap:5px;margin-top:9px}.l187-map-route:before{content:"";position:absolute;top:14px;right:8%;left:8%;height:2px;background:#294650}.l187-map-node{position:relative;z-index:1;min-width:0;text-align:center}.l187-map-node i{width:27px;height:27px;margin:auto;display:grid;place-items:center;border:2px solid #3c535c;border-radius:50%;background:#111f25;color:#708087;font:800 10px Rajdhani,sans-serif;font-style:normal}.l187-map-node b,.l187-map-node span{display:block}.l187-map-node b{margin-top:2px;color:#91a1a7;font-size:9px}.l187-map-node span{color:#60747c;font:4.5px "Share Tech Mono",monospace}.l187-map-node.complete i{border-color:#25db7c;background:#12543d;color:#afffd0}.l187-map-node.active i{border-color:#58e8ff;background:#126075;color:#fff;box-shadow:0 0 11px rgba(55,221,255,.55)}.l187-map-node.active b,.l187-map-node.active span{color:#9cedfb}.l187-map-node.boss i{border-radius:8px}.l187-map-node.boss.active i{border-color:#ffd166;background:#715319;box-shadow:0 0 11px rgba(255,209,102,.42)}'+
      '.l187-sector-next{display:flex;justify-content:space-between;gap:8px;margin-top:8px;padding-top:6px;border-top:1px solid rgba(255,255,255,.08);color:#6f8991;font:5px "Share Tech Mono",monospace}.l187-sector-next span:last-child{text-align:right;color:#c4a45c}'+
      '.l187-mission-summary{display:grid;grid-template-columns:1.25fr .75fr;gap:6px;margin-top:7px}.l187-mission-summary>div{padding:8px;border:1px solid rgba(255,255,255,.09);border-radius:9px;background:rgba(0,0,0,.2)}.l187-mission-summary small,.l187-mission-summary b{display:block}.l187-mission-summary small{color:#7e9299;font:5.5px "Share Tech Mono",monospace}.l187-mission-summary b{margin-top:2px;color:#fff;font-size:13px}.l187-mission-summary .rewards b{color:#ffd166;font-size:9px}'+
      '.l187-sector-transition{margin-top:9px;padding:10px;border:1px solid rgba(255,209,102,.48);border-radius:10px;background:linear-gradient(145deg,rgba(76,57,16,.62),rgba(5,24,30,.9));text-align:center}.l187-sector-transition small,.l187-sector-transition b,.l187-sector-transition span{display:block}.l187-sector-transition small{color:#ffd166;font:6px "Share Tech Mono",monospace}.l187-sector-transition b{margin-top:3px;color:#fff;font-size:15px}.l187-sector-transition span{margin-top:3px;color:#9dddeb;font:5.5px "Share Tech Mono",monospace}.l187-sector-transition div{display:flex;align-items:center;justify-content:center;margin-top:7px}.l187-sector-transition i{width:24px;height:24px;display:grid;place-items:center;border:2px solid;border-radius:50%;font-style:normal}.l187-sector-transition i.complete{border-color:#26df80;color:#8fffb9}.l187-sector-transition i.active{border-color:#56e6ff;color:#bdf7ff}.l187-sector-transition em{width:60px;height:2px;background:linear-gradient(90deg,#26df80,#56e6ff)}'+
      '@media(max-height:720px){#lsc137-app.l187-campaign-mode .l137-hero{height:19%;min-height:105px}.l187-sector-map{padding:8px}.l187-sector-map>p{display:none}.l187-campaign-mode .l187-campaign-dock{bottom:calc(54px + env(safe-area-inset-bottom,0px))}}'+
      '@media(max-width:380px){.l187-dock-copy{display:none}.l187-campaign-dock .l137-deploy{width:100%;min-width:0}.l187-sector-head h3{font-size:16px}}';
  ...
  function installCombatHudStyles(){
    ...
    style.textContent=
      ...
      '#l190-hq-hud.critical{border-color:#ff5a43;background:rgba(49,8,6,.95);box-shadow:0 0 18px rgba(255,55,37,.34)}#l190-hq-hud.critical .l190-hq-row{color:#ffb2a6}#l190-hq-hud.critical .l190-hq-fill{background:linear-gradient(90deg,#9d1010,#ff4b36)}'+
      '#l190-hq-hud.direct{border-color:#ff5a43;box-shadow:0 0 18px rgba(255,55,37,.42);animation:l190DirectAssault .42s ease-in-out infinite alternate}#l190-hq-hud.direct .l190-hq-row span:first-child{color:#ff6c58}@keyframes l190DirectAssault{to{filter:brightness(1.45);transform:scale(1.006)}}';
    document.head.appendChild(style);
  }
  ...
  function installUI() {
    ...
    id('l191-gc-launch').onclick=function(){if(window.LSCGameCenter)window.LSCGameCenter.showDashboard('dashboard').then(function(){syncGameCenterLaunch();});};
  ...
  var CAMPAIGN_SECTORS = [
    {name:'REDHAVEN OUTSKIRTS',objective:'Hold the evacuation road and keep Redhaven’s eastern approach open.',theater:'OVERRUN FORWARD OUTPOST'},
    {name:'BLACKRIDGE INDUSTRIAL',objective:'Defend the factory district before the infected reach the rail corridor.',theater:'COLLAPSED INDUSTRIAL SECTOR'},
    {name:'WESTBRIDGE QUARANTINE',objective:'Reinforce the quarantine line and prevent a breach into Westbridge.',theater:'RUINED URBAN PERIMETER'},
    {name:'EVACUATION CORRIDOR',objective:'Protect the final civilian withdrawal route through the outer city.',theater:'OVERRUN FORWARD OUTPOST'},
    {name:'RUINED CITY CENTER',objective:'Retake the civic district and establish a forward command position.',theater:'COLLAPSED INDUSTRIAL SECTOR'},
    {name:'GROUND ZERO',objective:'Advance into the infection core and destroy its command organisms.',theater:'RUINED URBAN PERIMETER'}
  ];
  function campaignSectorForPhase(phase){
    phase=Math.max(1,Math.floor(Number(phase)||1));
    var index=Math.floor((phase-1)/5),base=CAMPAIGN_SECTORS[Math.min(index,CAMPAIGN_SECTORS.length-1)];
    if(index<CAMPAIGN_SECTORS.length)return{index:index,number:index+1,start:index*5+1,end:index*5+5,name:base.name,objective:base.objective,theater:base.theater};
    var depth=index-CAMPAIGN_SECTORS.length+2;
    return{index:index,number:index+1,start:index*5+1,end:index*5+5,name:'GROUND ZERO · DEPTH '+depth,objective:'Push deeper into the infection core and hold the newly secured command line.',theater:CAMPAIGN_SECTORS[index%3].theater};
  }
  function campaignBossProfile(phase){
    var cycle=Math.floor((Math.max(1,Math.floor(Number(phase)||1))-1)/5)%3;
    return cycle===1?{grade:2,id:'juggernaut',name:'JUGGERNAUT'}:cycle===2?{grade:3,id:'outbreak',name:'OUTBREAK PRIME'}:{grade:1,id:'siege',name:'SIEGE BREAKER'};
  }
  function campaignMapMarkup(phase){
    var sector=campaignSectorForPhase(phase),next=campaignSectorForPhase(sector.end+1),nodes='';
    for(var node=sector.start;node<=sector.end;node++){
      var state=node<=meta.bestPhase?'complete':node===meta.phase?'active':'locked',boss=node===sector.end,selectable=node<=meta.bestPhase||node===meta.phase,tag=selectable?'button':'div';
      nodes+='<'+tag+' class="l187-map-node '+state+(boss?' boss':'')+(selectable?' selectable':'')+(node===phase?' selected':'')+'" '+(selectable?'data-campaign-phase="'+node+'"':'')+'><i>'+(state==='complete'?'✓':boss?'B':node-sector.start+1)+'</i><b>'+node+'</b><span>'+(boss?'BOSS':state==='complete'?'SECURED':state==='active'?'CURRENT':'LOCKED')+'</span></'+tag+'>';
    }
    var currentSector=campaignSectorForPhase(meta.phase),previous=sector.index>0,nextEnabled=sector.index<currentSector.index;
    return '<section class="l187-sector-map"><div class="l187-sector-head"><div><small>SECTOR '+sector.number+' · PHASES '+sector.start+'–'+sector.end+'</small><h3>'+sector.name+'</h3></div><strong>'+(Math.min(5,Math.max(1,phase-sector.start+1)))+' / 5</strong></div><p>'+sector.objective+'</p><div class="l187-map-route">'+nodes+'</div><div class="l189-sector-nav"><button data-sector-nav="previous" '+(previous?'':'disabled')+'>← PREVIOUS SECTOR</button><button data-sector-nav="next" '+(nextEnabled?'':'disabled')+'>NEXT SECTOR →</button></div><div class="l187-sector-next"><span>CURRENT THEATER · '+sector.theater+'</span><span>NEXT · '+next.name+'</span></div></section>';
  }
  function campaignTransitionMarkup(clearedPhase){
    if(clearedPhase%5!==0)return'';
    var secured=campaignSectorForPhase(clearedPhase),next=campaignSectorForPhase(clearedPhase+1);
    return '<div class="l187-sector-transition"><small>SECTOR '+secured.number+' SECURED</small><b>'+secured.name+'</b><span>COMMAND ADVANCING TO '+next.name+' · PHASE '+(clearedPhase+1)+'</span><div><i class="complete">✓</i><em></em><i class="active">→</i></div></div>';
  }
  ...
  function refreshHeader() {
    ...
    var visibleTier=Math.min(HQ_LEVEL_CAP,Math.max(1,meta.hq));
    id('l137-hq-lv').textContent = HQ_TIER_NAMES[visibleTier-1]+' · LEVEL ' + meta.hq;
    var art=document.querySelector('.l137-hq-art');if(art)art.setAttribute('data-tier',String(Math.min(HQ_LEVEL_CAP,Math.max(1,meta.hq))));
  ...
  function renderInventoryTab(panel){
    ...
    if(meta.equipmentNotice&&meta.equipmentNotice.type==='balance-cache'){
      var cacheItems=Array.isArray(meta.equipmentNotice.items)?meta.equipmentNotice.items.filter(function(item){return typeof item==='string';}).slice(0,3):[];
      if(cacheItems.length)notice='<div class="l167-equipment-notice"><b>BUILD 186 VETERAN CACHE</b><br>Your cleared campaign milestones earned '+cacheItems.join(' and ')+'. The items are locked for safekeeping; your active loadout was not changed.</div>';
      meta.equipmentNotice=null;saveMeta();
    }
    var equippedCards=EQUIPMENT_SLOTS.map(function(slot){
      var instance=equippedInstance(slot.id),definition=instance&&equipmentDefinition(instance.itemId);
      return '<div class="l167-equipped-slot '+(definition?'filled '+definition.rarity:'')+'" '+(instance?'data-equipment-select="'+instance.uid+'"':'')+'>'+equipmentIcon(slot.id)+'<span>'+(definition?equipmentRarity(definition.rarity).label+' · ':'')+slot.short+'</span><b>'+(definition?definition.name:'EMPTY SLOT')+'</b><small>'+(definition?definition.effectText:slot.description)+'</small>'+(definition?'<small class="l186-equipped-badge">EQUIPPED</small>':'')+'</div>';
    }).join('');
  ...
  function renderTab(tab,options) {
    ...
    p.classList.toggle('l205-panel',meta.settlementMode===204&&['campaigns','hq','commander','inventory','operations'].indexOf(tab)>=0);
    // Keep settlement screens outside the legacy shell's layout constraints.
    ...
    if (tab === 'campaign') {
      ...
      var campaignLabel=(replay?'REPLAY':'DEPLOY')+' PHASE '+selected+' · '+spend+' ENERGY',rewardView=replay?resourceMarkup('credits',previewCredits,'CREDITS'):resourcePair(previewCredits,previewParts),replayNotice=replay?'<div class="l189-replay-notice">TRAINING REPLAY · 40% CREDITS · NO TECH PARTS, EQUIPMENT, PROGRESSION, BADGES, OR LEADERBOARD CREDIT</div>':'';
  ...
  function createRun(settings) {
    ...
    if(!operation&&fortress.auxiliary){
      var auxDamage=7.5*(1+tech.turretDamage+gear.turretDamage),auxRange=205*s*(1+tech.turretRange+gear.turretRange);
      created.squad.push({source:'squad',x:cx-58*s,y:cy-10*s,r:9*s,damage:auxDamage,rate:2.15*(1+tech.turretRate+gear.turretRate),range:auxRange,cd:.18,parkAim:Math.PI});
      if(fortress.auxiliary>1)created.squad.push({source:'squad',x:cx+58*s,y:cy-10*s,r:9*s,damage:auxDamage,rate:2.15*(1+tech.turretRate+gear.turretRate),range:auxRange,cd:.42,parkAim:0});
    }
    if(junkyard){
  ...
  function enemy(kind) {
    ...
    var bossProfile=kind==='boss'&&!run.operation?campaignBossProfile(run.phase):null,bossGrade=bossProfile?bossProfile.grade:(kind==='boss'?1:0),bossArchetype=bossProfile?bossProfile.id:'siege';
    var unit={id:nextId,variant:variant,x:spawn.x,y:spawn.y,r:(kind==='boss'?34.5:kind==='armored'?17:13)*s,hp:base.hp*hpScale,maxHp:base.hp*hpScale,kind:kind,bossGrade:bossGrade,bossArchetype:kind==='boss'&&!run.operation?bossArchetype:null,speed:base.speed*s,damage:base.damage*damageScale,attackCycle:base.cycle,cd:0,age:Math.random(),moving:true,waiting:false,engaged:false,targetType:null,lane:lane.index,hit:0,flash:0,armor:0,maxArmor:0,armorHit:0,armorBroken:false,summonCd:bossArchetype==='outbreak'?5.5:0,summonWaves:0,aim:a+Math.PI};
    if(kind==='boss'&&!run.operation&&bossArchetype==='juggernaut'){
      unit.maxArmor=Math.ceil(unit.maxHp*(run.phase%5===0?.55:.38));
      unit.armor=unit.maxArmor;
      unit.speed*=.78;
    }
    if(kind==='boss'&&!run.operation&&bossArchetype==='siege'&&run.phase%5===0)unit.damage*=1.25;
  ...
  function applyDamage(target,amount,source){
    ...
    if(target.kind==='vehicle'&&source==='artillery'){amount*=2.6;target.artilleryHit=.42;}
    var dealt=0;
    if(target.armor>0){
      var armorMultiplier=source==='commander'?1+(run.commanderArmorDamage||0):source==='turret'?1+(run.turretArmoredDamage||0):source==='artillery'?1.15:1;
      var armorDamage=amount*armorMultiplier,absorbed=Math.min(target.armor,armorDamage),spill=Math.max(0,armorDamage-target.armor)/Math.max(.01,armorMultiplier);
      target.armor-=absorbed;target.armorHit=.2;dealt+=absorbed;
      pushParticle({x:target.x,y:target.y,life:.22,max:.22,r:15*dpr(),color:'#a9dce9',type:'armor-impact'});
      combatSfx('barrierHit',180);
      if(target.armor<=0&&!target.armorBroken){
        target.armor=0;target.armorBroken=true;run.feedback={text:'HEAVY ARMOR · BREACHED',source:'commander',life:1.7,max:1.7};run.destructionPulse=Math.max(run.destructionPulse||0,.34);
        for(var shard=0;shard<8;shard++)pushParticle({x:target.x+(Math.random()-.5)*target.r,y:target.y+(Math.random()-.5)*target.r,life:.45,max:.45,r:(3+Math.random()*4)*dpr(),color:'#9fc3cc',filled:true,type:'debris'});
        combatSfx('barrierBreak');combatHaptic('heavy',220);
      }
      amount=spill;
    }
    if(amount>0){var healthDamage=Math.min(target.hp,amount);target.hp-=amount;dealt+=healthDamage;}
    return dealt;
  }
  function pushParticle(particle){
    if(!run||!particle)return;
    var cap=Math.max(12,Number(run.performance&&run.performance.particleCap)||72),type=String(particle.type||''),critical=type==='artillery'||type==='fireball'||type==='shockwave'||type==='vehicle-explosion'||type==='command'||type==='hq-hit'||type==='barrier';
    ...
    particle.delay=Math.max(0,Number(particle.delay)||0);
    run.particles.push(particle);
  }
  function advanceParticles(dt){
    for(var p=run.particles.length-1;p>=0;p--){
      var particle=run.particles[p];
      if(particle.delay>0){particle.delay=Math.max(0,particle.delay-dt);continue;}
      particle.life-=dt;
      if(particle.life<=0)run.particles.splice(p,1);
    }
  }
  function spawnArtilleryImpact(target,sequence){
    var delay=Math.max(0,sequence||0)*.065,x=target.x,y=target.y,s=dpr();
    pushParticle({x:x,y:y,delay:delay,life:.30,max:.30,r:21*s,color:'#fff2a8',type:'artillery'});
    pushParticle({x:x,y:y,delay:delay+.035,life:.48,max:.48,r:31*s,color:'#ff5b25',type:'fireball'});
    pushParticle({x:x,y:y,delay:delay+.045,life:.62,max:.62,r:52*s,color:'#ffbd52',type:'shockwave'});
    pushParticle({x:x+(Math.random()-.5)*10*s,y:y+(Math.random()-.5)*10*s,delay:delay+.18,life:.72,max:.72,r:24*s,color:'#393b37',type:'smoke'});
    pushParticle({x:x+(Math.random()-.5)*18*s,y:y+(Math.random()-.5)*18*s,delay:delay+.06,life:.48,max:.48,r:5*s,color:'#ffb54f',filled:true,type:'debris'});
  }
  function spawnVehicleExplosion(e){
    var s=dpr();
    pushParticle({x:e.x,y:e.y,life:.62,max:.62,r:38*s,color:'#ff4d1f',type:'vehicle-explosion'});
    pushParticle({x:e.x,y:e.y,delay:.03,life:.62,max:.62,r:65*s,color:'#ffd166',type:'shockwave'});
    [[-18,4,34,'#ff7a25'],[16,-8,31,'#ffd05a'],[-4,-18,27,'#ff4d1f']].forEach(function(blast,index){pushParticle({x:e.x+blast[0]*s,y:e.y+blast[1]*s,delay:.08+index*.11,life:.64,max:.64,r:blast[2]*s,color:blast[3],type:'fireball'});});
    for(var n=0;n<12;n++)pushParticle({x:e.x+(Math.random()-.5)*e.r*1.3,y:e.y+(Math.random()-.5)*e.r*.8,delay:.1+n*.045,life:.72+Math.random()*.65,max:1.25,r:(n<7?4+Math.random()*6:20+Math.random()*17)*s,color:n<7?(n%2?'#ffd166':'#ff6a2c'):(n%2?'#252825':'#44342d'),filled:n<7,type:n<7?'debris':'smoke'});
  }
  function kill(i,e){
    ...
    if(!isVehicle)run.corpses.push({id:e.id,variant:e.variant,x:e.x,y:e.y,kind:e.kind,bossGrade:e.bossGrade||0,bossArchetype:e.bossArchetype||null,aim:e.aim,moving:false,waiting:false,engaged:false,life:corpseLife,max:corpseLife});
    ...
    if(isVehicle){
      ...
      run.vehicleDestructionTimer=2.15;run.destructionPulse=.95;
      run.feedback={text:'ARMORED TRANSPORT · DESTROYED',source:'artillery',life:1.4,max:1.4};
      spawnVehicleExplosion(e);
      combatSfx('impact');combatSfx('bossDown',110);combatHaptic('heavy',300);
    }else if(isBoss){
  ...
  function equipmentDropMarkup(award){
    ...
    var definition=award.definition,rarity=equipmentRarity(definition.rarity),slot=equipmentSlot(definition.slot),peer=equippedInstance(definition.slot),peerDefinition=peer&&equipmentDefinition(peer.itemId),powerDelta=definition.power-(peerDefinition?peerDefinition.power:0),capacityNote='STORED IN INVENTORY';
    ...
    return '<div class="l167-result-drop '+definition.rarity+'"><div class="l167-result-drop-head">'+equipmentIcon(definition.slot)+'<div class="l167-result-drop-copy"><small>'+(award.milestone?'MILESTONE REWARD':award.firstClear?'FIRST-CLEAR EQUIPMENT':'EQUIPMENT RECOVERED')+' · '+rarity.label+' · '+slot.short+'</small><b>'+definition.name+'</b><span>'+definition.effectText+'</span><div class="l168-compare"><span class="'+(powerDelta>0?'gain':powerDelta<0?'loss':'even')+'">POWER '+(powerDelta>0?'+':'')+powerDelta+'</span><span class="even">'+(peerDefinition?'VS '+peerDefinition.name.toUpperCase():'EMPTY '+slot.short+' SLOT')+'</span>'+equipmentComparisonEffects(definition,peerDefinition)+'</div><span>'+capacityNote+'</span></div></div><button id="l167-equip-drop" data-equipment-uid="'+award.instance.uid+'">EQUIP NOW · POWER +'+definition.power+'</button></div>';
  }
  function reportCompetitiveClear(details){
    var gameCenter=window.LSCGameCenter;if(!gameCenter||!details||!details.won)return;
    if(details.operation){
      if(!details.rewarded)return;
      gameCenter.reportOperationLevel(details.kind,details.level,true);
      if(details.kind==='containment'&&details.level>=5)gameCenter.unlock('containmentSpecialist',true);
      if(details.kind==='junkyard'&&details.level>=5)gameCenter.unlock('convoyCrusher',true);
      return;
    }
  ...
  function finish(won) {
    ...
    id('l137-result-kicker').textContent = operation?(operationKind==='junkyard'?(won?'JUNKYARD RECOVERY COMPLETE':'JUNKYARD RECOVERY FAILED'):(won?'DAILY OPERATION COMPLETE':'DAILY OPERATION FAILED')):(replay?(won?'TRAINING REPLAY COMPLETE':'TRAINING REPLAY FAILED'):won?(sectorCleared?'SECTOR SECURED':'MISSION ACCOMPLISHED'):'MISSION FAILED');
    id('l137-result-title').textContent = operation?(operationKind==='junkyard'?(won?'ARMORED CONVOY DESTROYED':'TARGET ESCAPED'):(won?'CONTAINMENT LEVEL '+operationLevel+' SECURED':'CONTAINMENT LEVEL '+operationLevel+' LOST')):(won?(sectorCleared?clearedSector.name+' HELD':'PHASE '+clearedPhase+' SECURED'):'HEADQUARTERS LOST');
    id('l137-result-copy').textContent = operation?(operationKind==='junkyard'?(won?(operationRewarded?'Recovery Level '+operationLevel+' is complete and today’s shared operation reward is claimed. Level '+nextOperationLevel+' is ready.':'The target was destroyed, but today’s shared operation reward was already claimed.'):'The armored transport reached extraction with '+formatNumber(vehicleArmor)+' armor remaining. Recovery Level '+operationLevel+' remains open.'):(won?(operationRewarded?'Containment Level '+operationLevel+' is secure and today’s shared operation reward is claimed. Level '+nextOperationLevel+' is ready.':'Containment was secured, but today’s shared operation reward was already claimed.'):'The infected breached Containment Level '+operationLevel+'. This level remains open until its first successful clear.')):(replay?(won?'Training replay complete. Reduced credits awarded; progression and competitive records are unchanged.':'Training replay failed. No resources or progression were awarded.'):(won?(clearedBoss.name+' is destroyed. '+(sectorCleared?'Command is advancing to '+campaignSectorForPhase(clearedPhase+1).name+'.':'Phase '+meta.phase+' is ready for deployment.')):defeatAdvice()+supportCopy));
  ...
  function summonOutbreakReinforcements(boss){
    if(!boss||boss.summonWaves>=4)return;
    var count=run.phase%5===0?3:2;
    for(var index=0;index<count;index++)run.enemies.push(enemy(index===0?'armored':index%2?'runner':'grunt'));
    boss.summonWaves++;boss.summonCd=Math.max(5.5,9-boss.summonWaves*.55);
    run.feedback={text:'OUTBREAK PRIME · REINFORCEMENTS',source:'hq',life:1.5,max:1.5};
    combatSfx('bossAlarm');combatHaptic('medium',300);
  }
  function updateBarrierStress(){
    run.lanes.forEach(function(lane){
      var barrier=lane.barricade,ratio=barrier.hp/Math.max(1,barrier.maxHp),point=lanePoint(lane,BARRICADE_WORLD_RADIUS,0);
      if(barrier.hp>0&&barrier.broken)barrier.broken=false;
  ...
  function updateJunkyard(dt){
    ...
    if(run.vehicleDestroyed){run.vehicleDestructionTimer=Math.max(0,(run.vehicleDestructionTimer||0)-dt);if(run.vehicleDestructionTimer<=0)finish(true);return;}
  ...
  function update(dt){
    ...
    run.lastStandActive=Math.max(0,(run.lastStandActive||0)-dt);
    run.artilleryPulse=Math.max(0,(run.artilleryPulse||0)-dt);
    run.destructionPulse=Math.max(0,(run.destructionPulse||0)-dt);
  ...
  function drawProductionHQ(h){var s=dpr(),level=Math.min(HQ_LEVEL_CAP,Math.max(1,Number(h.level)||1)),visual=Math.min(7,level),wallW=(50+(visual-1)*7)*s,wallH=(37+(visual-1)*5)*s;ctx.save();ctx.translate(h.x,h.y+5*s);ctx.fillStyle=level>=6?'rgba(37,72,76,.78)':level>=4?'rgba(45,83,86,.72)':'rgba(61,76,67,.64)';ctx.strokeStyle=level>=8?'#ffd166':level>=5?'#7ef8ff':level>=3?'#d4b45e':'#9a8653';ctx.lineWidth=(1+visual)*s;ctx.fillRect(-wallW,-wallH,wallW*2,wallH*2);ctx.strokeRect(-wallW,-wallH,wallW*2,wallH*2);if(level>=3){[[-wallW,-wallH],[wallW,-wallH],[-wallW,wallH],[wallW,wallH]].forEach(function(point){circle(point[0],point[1],(4+visual)*s,level>=5?'#2e686e':'#526b68',level>=8?'#ffd166':level>=5?'#9cecff':'#d4b45e',2*s);});}if(level>=7)circle(-wallW-13*s,0,9*s,'#173943','#8fefff',3*s);if(level>=9)circle(wallW+13*s,0,9*s,'#173943','#8fefff',3*s);ctx.restore();if(!spriteCell(4,h.x,h.y+43*s,(128+(visual-1)*4)*s,(120+(visual-1)*4)*s))drawHQ(h);}
  ...
  function drawProductionPerimeter(h){var s=dpr(),level=Math.min(5,Math.max(1,Number(h.level)||1));if(!combatAtlas.complete||!combatAtlas.naturalWidth)return drawPerimeter(h);run.lanes.forEach(function(lane){if(lane.barricade.hp<=0)return;var point=lanePoint(lane,BARRICADE_WORLD_RADIUS,0),ratio=lane.barricade.hp/Math.max(1,lane.barricade.maxHp),alpha=lane.barricade.flash>0?.58:1,width=(66+(level-1)*4)*s,height=(42+(level-1)*5)*s;ctx.save();ctx.translate(point.x,point.y);ctx.rotate(lane.rotation);if(lane.barricade.stress>0)ctx.translate((Math.random()-.5)*3*s,(Math.random()-.5)*2*s);spriteCell(6,0,12*s,width,height,alpha);if(level>=2){ctx.strokeStyle=level>=5?'#7ef8ff':level>=4?'#86a5a5':'#c3ad70';ctx.lineWidth=Math.max(1,level-1)*s;ctx.strokeRect(-width*.48,-height*.35,width*.96,height*.28);}if(ratio<.7){ctx.strokeStyle=ratio<.35?'#ff714f':'rgba(18,25,28,.9)';ctx.lineWidth=2*s;ctx.beginPath();ctx.moveTo(-width*.24,-height*.22);ctx.lineTo(-width*.06,-height*.05);ctx.lineTo(-width*.18,height*.12);if(ratio<.35){ctx.moveTo(width*.18,-height*.25);ctx.lineTo(width*.05,-height*.04);ctx.lineTo(width*.22,height*.14);}ctx.stroke();}ctx.restore();});}
  ...
  // Build 185 2D fallback mirrors the three authored campaign theaters.
  drawEnvironment=function(W,H,h){
    var s=dpr(),tier=1+(Math.floor((Math.max(1,run.phase)-1)/5)%3),palette=tier===3?['#111315','#343638','#272a2c']:tier===2?['#151814','#3d4238','#2d312e']:['#111710','#46573d','#343e31'];
    ctx.fillStyle=palette[0];ctx.fillRect(0,0,W,H);
    var glow=ctx.createRadialGradient(h.x,h.y,28*s,h.x,h.y,Math.max(W,H)*.72);glow.addColorStop(0,palette[1]);glow.addColorStop(.58,tier===3?'#272a2b':tier===2?'#30362f':'#25301f');glow.addColorStop(1,palette[0]);ctx.fillStyle=glow;ctx.fillRect(0,0,W,H);
    ctx.save();ctx.translate(h.x,h.y);ctx.strokeStyle=palette[2];ctx.lineWidth=30*s;for(var a=0;a<TAU;a+=Math.PI/2){ctx.beginPath();ctx.moveTo(Math.cos(a)*110*s,Math.sin(a)*110*s);ctx.lineTo(Math.cos(a)*Math.max(W,H),Math.sin(a)*Math.max(W,H));ctx.stroke();}ctx.strokeStyle='rgba(10,12,11,.72)';ctx.lineWidth=2*s;ctx.setLineDash([16*s,24*s]);for(var crack=0;crack<4;crack++){ctx.beginPath();ctx.moveTo((crack-1.5)*22*s,-Math.max(W,H));ctx.lineTo((crack-1.2)*18*s,Math.max(W,H));ctx.stroke();}ctx.restore();
    if(tier===1){
      [[.08,.17],[.79,.14],[.08,.78],[.78,.80]].forEach(function(p,index){ctx.fillStyle=index%2?'#4c5e52':'#766a47';ctx.fillRect(W*p[0],H*p[1],58*s,16*s);ctx.strokeStyle='#96875b';ctx.strokeRect(W*p[0],H*p[1],58*s,16*s);});
      ctx.strokeStyle='rgba(136,151,143,.55)';ctx.lineWidth=2*s;ctx.setLineDash([5*s,4*s]);ctx.strokeRect(12*s,H*.32,42*s,H*.35);ctx.strokeRect(W-54*s,H*.30,42*s,H*.36);ctx.setLineDash([]);
    }else if(tier===2){
      [[.04,.14,72,29,'#6b4630'],[.75,.18,74,30,'#34585a'],[.03,.76,82,31,'#59452f']].forEach(function(p){ctx.fillStyle=p[4];ctx.fillRect(W*p[0],H*p[1],p[2]*s,p[3]*s);ctx.strokeStyle='#262c29';for(var rib=8;rib<p[2];rib+=12){ctx.beginPath();ctx.moveTo(W*p[0]+rib*s,H*p[1]);ctx.lineTo(W*p[0]+rib*s,H*p[1]+p[3]*s);ctx.stroke();}});
      ctx.fillStyle='#4e5551';ctx.fillRect(W*.84,H*.58,32*s,82*s);ctx.fillRect(W*.02,H*.42,29*s,72*s);circle(W*.84+16*s,H*.58,16*s,'#59605c');circle(W*.02+14*s,H*.42,14*s,'#59605c');
    }else{
      [[0,.10,.22,.26],[.75,.11,.25,.29],[0,.72,.24,.28],[.78,.70,.22,.30]].forEach(function(p,index){ctx.fillStyle=index%2?'#494b49':'#52504a';ctx.fillRect(W*p[0],H*p[1],W*p[2],H*p[3]);ctx.fillStyle='#111514';for(var window=0;window<3;window++)ctx.fillRect(W*p[0]+(12+window*22)*s,H*p[1]+18*s,10*s,15*s);});
      ctx.fillStyle='#282b29';ctx.save();ctx.translate(W*.76,H*.54);ctx.rotate(-.45);ctx.fillRect(-30*s,-12*s,60*s,24*s);ctx.restore();
    }
    if(run.assault>=2){for(var impact=0;impact<3;impact++){var ix=(.22+impact*.28)*W,iy=(impact%2?.26:.73)*H;circle(ix,iy,(10+impact*2)*s,'rgba(20,22,20,.72)');ctx.fillStyle='rgba(63,66,61,.18)';circle(ix,iy-18*s,(16+impact*3)*s,'rgba(63,66,61,.18)');}}
    if(run.assault>=3){ctx.fillStyle='rgba(255,70,20,.72)';circle(W*.17,H*.54,7*s,'rgba(255,70,20,.72)');circle(W*.83,H*.43,8*s,'rgba(255,70,20,.72)');}
    ctx.save();ctx.strokeStyle='rgba(219,205,140,.22)';ctx.lineWidth=8*s;ctx.setLineDash([18*s,7*s]);run.lanes.forEach(function(item,index){var p=lanePoint(item,BARRICADE_WORLD_RADIUS,0);if(index===0){ctx.beginPath();ctx.moveTo(p.x,p.y);}else ctx.lineTo(p.x,p.y);});ctx.closePath();ctx.stroke();ctx.restore();drawProductionPerimeter(h);
  };
  ...
  function drawArmoredVehicle(e,wreck){
    var s=dpr(),damage=clamp(1-e.hp/Math.max(1,e.maxHp),0,1),body=wreck?'#2b2420':damage>.65?'#403b32':damage>.32?'#514a3b':'#635640',trim=wreck?'#171514':'#302e29';
  ...
  function drawPerimeter(h){var s=dpr();run.lanes.forEach(function(lane){if(lane.barricade.hp<=0)return;var point=lanePoint(lane,BARRICADE_WORLD_RADIUS,0),x=point.x,y=point.y,ratio=lane.barricade.hp/Math.max(1,lane.barricade.maxHp);ctx.save();ctx.translate(x,y);ctx.rotate(lane.rotation);if(lane.barricade.stress>0)ctx.translate((Math.random()-.5)*3*s,0);ctx.fillStyle=lane.barricade.flash>0?'#a24736':ratio<.35?'#6d352e':'#5d5138';ctx.fillRect(-27*s,-6*s,54*s,12*s);ctx.fillStyle='#887450';ctx.fillRect(-25*s,-6*s,50*s,4*s);ctx.strokeStyle='rgba(255,216,139,.3)';ctx.strokeRect(-27*s,-6*s,54*s,12*s);if(ratio<.7){ctx.strokeStyle=ratio<.35?'#ff714f':'#1b2022';ctx.lineWidth=2*s;ctx.beginPath();ctx.moveTo(-13*s,-5*s);ctx.lineTo(-3*s,1*s);ctx.lineTo(-10*s,6*s);if(ratio<.35){ctx.moveTo(10*s,-5*s);ctx.lineTo(3*s,0);ctx.lineTo(12*s,6*s);}ctx.stroke();}ctx.restore();});}
  ...
  function drawBoss(e){
    var s=dpr(),ang=Math.atan2(run.hq.y-e.y,run.hq.x-e.x),profile=e.bossArchetype?campaignBossProfile(run.phase):{id:'siege',name:'CONTAINMENT ALPHA'},type=e.bossArchetype||'siege',body=type==='juggernaut'?'#52615a':type==='outbreak'?'#56394e':'#6d3a2c',edge=type==='juggernaut'?'#b7d0c8':type==='outbreak'?'#ef79c4':'#ff9b4b';
    ctx.save();ctx.translate(e.x,e.y);ctx.rotate(ang+Math.PI/2);ctx.fillStyle='rgba(0,0,0,.55)';ctx.beginPath();ctx.ellipse(0,17*s,27*s,8*s,0,0,TAU);ctx.fill();poly([[-21*s,16*s],[21*s,16*s],[18*s,-10*s],[11*s,-22*s],[-11*s,-22*s],[-18*s,-10*s]],body,edge,2*s);
    if(type==='juggernaut'){ctx.fillStyle='#26343a';ctx.fillRect(-16*s,-9*s,32*s,16*s);ctx.strokeStyle='#9eb4af';ctx.lineWidth=3*s;ctx.strokeRect(-15*s,-8*s,30*s,14*s);ctx.fillStyle='#70817a';ctx.beginPath();ctx.ellipse(-18*s,-6*s,8*s,6*s,-.25,0,TAU);ctx.ellipse(18*s,-6*s,8*s,6*s,.25,0,TAU);ctx.fill();if(e.armor<=0){ctx.strokeStyle='#ffb14a';ctx.beginPath();ctx.moveTo(-10*s,-7*s);ctx.lineTo(5*s,7*s);ctx.moveTo(8*s,-7*s);ctx.lineTo(-2*s,5*s);ctx.stroke();}}
    else if(type==='outbreak'){ctx.fillStyle='#211621';ctx.fillRect(-15*s,-8*s,30*s,15*s);ctx.fillStyle='#d5529e';[[-13,-11],[0,-16],[13,-10],[-8,4],[9,5]].forEach(function(g){circle(g[0]*s,g[1]*s,4*s,'#d5529e','#ff9ed7',s);});ctx.strokeStyle='#a9427f';ctx.lineWidth=3*s;ctx.beginPath();ctx.moveTo(-15*s,-6*s);ctx.lineTo(-23*s,-24*s);ctx.moveTo(0,-12*s);ctx.lineTo(0,-30*s);ctx.moveTo(15*s,-6*s);ctx.lineTo(23*s,-24*s);ctx.stroke();}
    else{ctx.fillStyle='#2a1b18';ctx.fillRect(-16*s,-8*s,32*s,16*s);ctx.fillStyle='#ff833d';ctx.fillRect(-13*s,-13*s,7*s,5*s);ctx.fillRect(6*s,-13*s,7*s,5*s);ctx.strokeStyle='#e7a04e';ctx.lineWidth=4*s;ctx.beginPath();ctx.moveTo(-14*s,-2*s);ctx.lineTo(-21*s,-23*s);ctx.moveTo(14*s,-2*s);ctx.lineTo(21*s,-23*s);ctx.stroke();}
    ctx.restore();ctx.fillStyle=edge;ctx.font='bold '+8*s+'px "Share Tech Mono"';ctx.textAlign='center';ctx.fillText(profile.name,e.x,e.y+32*s);
  }
  ...
  function drawCombatParticle(p){
    if((p.delay||0)>0)return;
    var s=dpr(),ratio=clamp(p.life/Math.max(.01,p.max||1),0,1),phase=1-ratio;
    ctx.save();ctx.globalAlpha=ratio;
    if(p.type==='artillery'){
      ctx.strokeStyle='#fff1a6';ctx.lineWidth=Math.max(4,13*ratio)*s;ctx.shadowColor='#ff5a1f';ctx.shadowBlur=24*s;ctx.beginPath();ctx.moveTo(p.x,p.y-p.r*(3.5+ratio));ctx.lineTo(p.x,p.y);ctx.stroke();circle(p.x,p.y,p.r*(.26+phase*.42),'#fff0a0');
    }else if(p.type==='shockwave'){
      circle(p.x,p.y,p.r*(.36+phase*1.25),null,p.color,Math.max(1,7*ratio)*s);
    }else if(p.type==='fireball'||p.type==='vehicle-explosion'){
      var blast=p.type==='vehicle-explosion'?1.2:1;ctx.shadowColor='#ff3c16';ctx.shadowBlur=30*s;circle(p.x,p.y,p.r*blast*(.26+phase*.95),p.color);ctx.globalAlpha=ratio*.86;circle(p.x,p.y,p.r*blast*(.13+phase*.48),'#ffd56a');
    }else if(p.type==='smoke'){
      ctx.globalAlpha=ratio*.42;circle(p.x+Math.sin(phase*8)*p.r*.12,p.y-phase*p.r*1.4,p.r*(.48+phase*.68),p.color);
    }else if(p.filled)circle(p.x,p.y,p.r*(1.2-ratio*.2),p.color);
    else circle(p.x,p.y,p.r*(1-ratio*.35),null,p.color,Math.max(1,4*ratio)*s);
    ctx.restore();
  }
  function draw(){
