...
clear(m,'1,0');clear(m,'2,0');assert.equal(m.credits,212,'no town discovery bounty');assert.equal(r.ready(m,1),true);assert.equal(r.rate(m),2,'connected alone gives no town income');assert.equal(r.win(m,1),true);assert.equal(r.rate(m),4);assert.equal(r.win(m,1),false);assert.equal(r.ready(m,1),false);
...
const levels=new Set();for(let n=1;n<=81;n++){levels.add(r.stars(n));assert.equal(r.combat(n),1+(r.stars(n)-1)*5);}assert.equal(levels.size,5);
